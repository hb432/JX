   ...
   jx.all(function (player) {
      ...
      jx.ar(instance.effectivePermissions).forEach(function (info) {
         try {
            ...
            if (jx.ty(err) !== 'java.lang.IllegalArgumentException') throw err;
            var match = 'Given attachment is not part of Permissible object CraftPlayer{name=' + instance.name + '}';
            if (err.message !== match) throw err;
         }
      });
      var groups = jx.data.server('group');
